...
bool Eposide::is_last_file(){
    if(_cur_file >= _file_list.size() - 1)
        return true;
...
std::string Eposide::get_cur_filename_inc(){
    if(!is_last_file() )
        return "N_A";

    return get_cur_filename();
}

std::string Eposide::get_cur_filename(){
    return _file_list[_cur_file];
}
